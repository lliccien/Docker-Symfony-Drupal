...
RUN sed -i '$ a\opcache.max_accelerated_files=20000' /etc/php5/mods-available/opcache.ini && \
    ...
    sed -i '$ a\opcache.validate_timestamps=0' /etc/php5/mods-available/opcache.ini && \
    sed -i '$ a\opcache.fast_shutdown=1' /etc/php5/mods-available/opcache.ini && \
...
RUN sed -ri 's/^max_execution_time\s*=\s*30/max_execution_time = 3600/g' /etc/php5/cli/php.ini && \
    ...
    echo 'extension=uploadprogress.so' >> /etc/php5/cli/php.ini
