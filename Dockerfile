...
# config Xdebug
RUN sed -i '$ a\xdebug.var_display_max_depth=4' /etc/php/7.1/mods-available/xdebug.ini && \
...
RUN sed -ri 's/^max_execution_time\s*=\s*30/max_execution_time = 3600/g' /etc/php/7.1/apache2/php.ini && \
    ...
    sed -ri 's/^upload_max_filesize\s*=\s*2M/upload_max_filesize = 2048M/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.enable=0/opcache.enable=1/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.enable_cli=0/opcache.enable_cli=1/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.memory_consumption=64/opcache.memory_consumption=128/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.interned_strings_buffer=4/opcache.interned_strings_buffer=8/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.max_accelerated_files=2000/opcache.max_accelerated_files=4000/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.revalidate_freq=2/opcache.revalidate_freq=60/g' /etc/php/7.1/apache2/php.ini && \
    sed -ri 's/^;opcache.fast_shutdown=0/opcache.fast_shutdown=1/g' /etc/php/7.1/apache2/php.ini && \
    echo 'extension=uploadprogress.so' >> /etc/php/7.1/apache2/php.ini

# Confugure php.ini php Cli
RUN sed -ri 's/^max_execution_time\s*=\s*30/max_execution_time = 3600/g' /etc/php/7.1/cli/php.ini && \
    ...
    sed -ri 's/^upload_max_filesize\s*=\s*2M/upload_max_filesize = 2048M/g' /etc/php/7.1/cli/php.ini

# Config apache RUN a2enmod rewrite
RUN echo "ServerName localhost" >> /etc/apache2/apache2.conf && a2enmod rewrite php7.1
