...
# Install Node.js
RUN curl -sL https://deb.nodesource.com/setup_6.x -o && apt-get install -y nodejs
...
RUN sed -ri 's/^max_execution_time\s*=\s*30/max_execution_time = 3600/g' /etc/php5/apache2/php.ini && \
    ...
    sed -ri 's/^;opcache.enable=0/opcache.enable=1/g' /etc/php5/apache2/php.ini && \
    sed -ri 's/^;opcache.enable_cli=0/opcache.enable_cli=1/g' /etc/php5/apache2/php.ini && \
    sed -ri 's/^;opcache.memory_consumption=64/opcache.memory_consumption=128/g' /etc/php5/apache2/php.ini && \
